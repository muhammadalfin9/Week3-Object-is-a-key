...
function countProfit(shoppers) {
  ...
  for (let j = 0; j < listBarang.length; j++) {
    let barang = listBarang[j][0];
    let harga = listBarang[j][1];
    let stok = listBarang[j][2];

    let laporan = {};
    let pelanggan = [];
    laporan.product = barang;
    laporan.shoppers = pelanggan;
    laporan.leftOver = stok;
    laporan.totalProfit = 0;
    for (let i = 0; i < shoppers.length; i++) {
      if (
        shoppers[i].product == laporan.product &&
        laporan.leftOver >= shoppers[i].amount
      ) {
        pelanggan.push(shoppers[i].name);
        laporan.leftOver -= shoppers[i].amount;
        laporan.totalProfit += harga * shoppers[i].amount;
      }
    }
    hasil.push(laporan);
  }
